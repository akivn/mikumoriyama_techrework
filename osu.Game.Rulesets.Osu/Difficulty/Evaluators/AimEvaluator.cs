...
namespace osu.Game.Rulesets.Osu.Difficulty.Evaluators
{
    public static class AimEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current, bool withSliderTravelDistance)
        {
            ...
            if (Math.Max(osuCurrObj.StrainTime, osuLastObj.StrainTime) < 1.25 * Math.Min(osuCurrObj.StrainTime, osuLastObj.StrainTime)) // If rhythms are the same.
            {
                if (osuCurrObj.Angle != null && osuLastObj.Angle != null && osuLastLastObj.Angle != null)
                {
                    ...
                    angleDeviationBonus *= Math.Max(150 / Math.Max(osuCurrObj.StrainTime, 25) - 1, 0) // Buff starts at 200bpm 1/2s.
                                        * (currVelocity != 0 ? ((Math.PI - Math.Abs(currAngle)) / Math.PI) : 0); // Buff based on angles or overlap cheeses.
                    wiggleBonus *= Math.Max(150 / Math.Max(osuCurrObj.StrainTime, 62.5) - 1, 0) // Buff starts at 200bpm 1/2s and caps at 240bpm 1/4s.
                                * Math.Min(1 + osuCurrObj.LazyJumpDistance / 120, 2) // Buff based on LazyJumpDistance. This part caps at 2x at 120o!px.
                                * ((Math.Max((prevVelocity + 0.7) / (currVelocity + 0.7), (currVelocity + 0.7) / (prevVelocity + 0.7))) - 0.95) * Math.Min(Math.Pow(Math.Min(prevVelocity, currVelocity), 3), 1) // Buff based on velocity change factor and initial velocity, capping at 1o!px/ms.
                                * Math.Pow(Math.Sin(Math.Max(Math.Abs(currAngle - lastAngle), Math.Abs(lastAngle - currAngle))), 4) // Preventing streams from overbuffing due to this formula also buff straight streams.
                                * (osuCurrObj.LazyJumpDistance != 0 ? Math.Pow(Math.Sin(Math.Abs(currAngle) / 2), 2) : 0); // Buffing wiggles properly.
            ...
            // Multiply acute / wide angle bonus by Angle Deviation bonus and Wiggle Bonus.
